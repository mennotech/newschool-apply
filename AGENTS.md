...
- **What AI Agents Should Do**:
  ...
  - **Generate Drupal Field Definitions From Schema**: For backend content-type and field-definition changes, load the `DrX-Schema` PowerShell module from the repo root with `pwsh -File Enable-DrX-Schema.ps1`, then run `Export-DrXDrupalScaffoldConfig` to generate Drupal field definition YAML. Treat generated field definition files as scaffold output, not hand-authored source.
  - **Handle Field Definition Formatting At The Source**: If generated field definition YAML needs reformatting or structural changes, update the schema inputs and/or add functionality in `schema/DrX-Schema/DrX-Schema.psm1`, then regenerate with `Export-DrXDrupalScaffoldConfig`. Do not manually reformat generated field definition files as the primary fix.
